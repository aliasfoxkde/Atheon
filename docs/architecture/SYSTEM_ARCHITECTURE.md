...
├── patterns.bundle      (Embedded pattern database — generated by bundler/)
├── bundle.go            (Bundle loading + gzip decompression)
├── pattern.go           (Pattern type + matching logic)
├── pattern_state.go     (Enable/disable per-pattern persistence)
├── runner.go            (Directory walker + concurrency)
├── finding.go           (Finding type + SARIF output)
└── ignore.go            (Ignore rules — .atheonignore + .gitignore)

community/               (Pattern contributions — YAML source for bundler)
├── secrets/             (API keys, tokens, credentials)
├── pii/                 (Personal identifiable information)
├── code-quality/        (Debug artifacts, hardcoded values)
├── devops/              (CI/CD bypass flags, K8s/Docker misconfigs)
├── ai-detection/        (AI-generated code markers, prompt injection)
└── ...                  (plus security-hardening, web-security, compliance)
```

## 🎯 Success Metrics

### **Quality Indicators**
- **Test Coverage**: Maintain ≥70% (CI gate); current ≥97%
- **Pattern Count**: 250+ patterns (current: ~255 across 18 active categories)
